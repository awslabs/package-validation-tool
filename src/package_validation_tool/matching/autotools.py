...
# Main mirror URL for downloading Autotools packages
GNU_FTP_BASE_URL = "https://www.mirrorservice.org/sites/ftp.gnu.org/gnu/"
# Alternative mirror URL for downloading Autotools packages (some versions only available in alpha)
GNU_FTP_ALPHA_URL = "https://www.mirrorservice.org/sites/alpha.gnu.org/gnu/"
...
TOOL_CONFIGS: Dict[str, Dict[str, Any]] = {
    "automake": {
        ...
        "detection_files": [
            ("aclocal.m4", r"generated automatically by aclocal (\S+)"),
            ("Makefile.in", r"generated by automake (\S+)"),
        ],
        "sha256_hashsums": {
    ...
    "autoconf": {
        ...
        "detection_files": [("configure", r"Generated by GNU Autoconf (\S+)")],
        "sha256_hashsums": {
            ...
            "autoconf-2.69b.tar.gz": "b87aca642b6f29dc78f0584a475524d0b4eb50e054702f54921f8169a43c1c3e",
            "autoconf-2.69c.tar.gz": "6d778f6228e536ac962c597f2b1fb9ddf6669b7cbd1cb80b7c9c6db2dd6f4916",
            "autoconf-2.69d.tar.gz": "64e031c45a89658d0551bd5ae6222bd36481d635772ffe7dae1dca28bdb70357",
            "autoconf-2.69e.tar.gz": "af53e6fb86369cb4123cab806e62eada1a04c4a454b8848bf26317d76b5b50f6",
            "autoconf-2.72c.tar.gz": "21b64169c820c6cdf27fc981ca9c2fb615546e5dead92bccf8d92d0784cdd364",
            "autoconf-2.72d.tar.gz": "c09dcba3d051507459df2fcd58d6f19e5b342568fa910e3bb3a74b4402cde3a6",
            "autoconf-2.72e.tar.gz": "f3478d3b597d51f5d61596fb2f6f6aba49cdd974b4b05ff0bac57f56b5cfdb39",
        },
    },
    "gettext": {
        "default_version": DEFAULT_GETTEXT_VERSION,
        "detection_files": [
            ("gettext.m4", r"gettext-(\S+)"),
            ("m4/gettext.m4", r"gettext-(\S+)"),
        ],
        "sha256_hashsums": {
    ...
    "libtool": {
        ...
        "detection_files": [
            ("ltmain.sh", r"libtool \(GNU libtool\) (\S+)"),
            ("build-aux/ltmain.sh", r"libtool \(GNU libtool\) (\S+)"),
            ("config/ltmain.sh", r"libtool \(GNU libtool\) (\S+)"),
        ],
        "sha256_hashsums": {
            ...
            "libtool-2.4.2.418.tar.gz": "67ba4b1bc1f5a1809b967fdc0d72ea3e11f4979f67f4d7fbf6100e8197af0d65",
        },
    },
}
...
class AutotoolsRunner:
    ...
    def _detect_autotools_versions(self) -> Dict[str, Optional[str]]:
        ...
        for tool, config in TOOL_CONFIGS.items():
            ...
            if version:
                # Remove trailing non-alphanumeric characters (dots, commas, brackets, etc.)
                version = re.sub(r"[^a-zA-Z0-9]+$", "", version)
                # Remove "-dirty" suffix (it signals that a version was used that is git-repo-based
                # and that git repo had uncommited changes, due to applied downstream patches)
                if version.endswith("-dirty"):
                    version = version[:-6]  # Remove last 6 characters ('-dirty')
    ...
    def _download_package(self, package_name: str, version: str) -> bool:
        ...
        # TODO: Add download and build of git-based versions like libtool-2.4.6.42-b88ce

        # TODO: Add detection of downstream patches to apply, collect the patches, and modify the
        #       tool-version/ path so that caching layer doesn't confuse patches-applied vs
        #       patches-not-applied versions of Autotools
        ...
        # Try both URLs with full retry attempts for each
        urls = [
            f"{GNU_FTP_BASE_URL}{package_name}/{archive_name}",
            f"{GNU_FTP_ALPHA_URL}{package_name}/{archive_name}",
        ]

        for url_index, url in enumerate(urls):
            url_name = "primary" if url_index == 0 else "alternative"
            log.info("Trying %s URL: %s", url_name, url)

            if not is_url_accessible(url):
                log.warning("URL is not accessible: %s", url)
                continue
            ...
            for attempt in range(max_retries + 1):
                if attempt > 0:
                    log.info(
                        "Retrying download from %s URL (attempt %d/%d)...",
                        url_name,
                        attempt + 1,
                        max_retries + 1,
                    )
                ...
                if download_file(url, str(local_path)):
                    log.info("Successfully downloaded %s from %s URL", archive_name, url_name)
                    # Verify checksum after successful download
                    if not self._verify_checksum(package_name, archive_name, local_path):
                        log.error(
                            "Checksum verification failed for downloaded file %s", archive_name
                        )
                        try:
                            local_path.unlink()
                        except Exception as e:
                    ...
                    log.warning(
                        "Download attempt %d failed for %s from %s URL",
                        attempt + 1,
                        archive_name,
                        url_name,
                    )
        ...
        log.error(
            "Failed to download %s from all URLs after trying all retry attempts", archive_name
        )
        return False
