...
# Split from the main test_suggest_package_archives.py. Tests an "empty" package, i.e. a
# package that doesn't contain any archives. See also testrpm_for_suggesting_empty.spec.
...
TESTRPM_SPEC_FILE = Path(TEST_DIR_PATH) / "artefacts" / "rpm" / "testrpm_for_suggesting_empty.spec"
...
def prepare_srpm_test_environment(target_dir: str):
    ...
    src_rpm_file = Path(target_dir) / "testrpm_for_suggesting_empty.src.rpm"
    src_rpm_file.touch()
    ...
    return srpm_content_path, src_rpm_file


def test_srpm_suggest_package_archives_empty_cli():
    """Test srpm suggest-package-archives CLI on an empty package."""
    with tempfile.TemporaryDirectory() as temp_dir, pushd(temp_dir):
        found_accessible_url = False
        srpm_content_path, src_rpm_file = prepare_srpm_test_environment(temp_dir)
        ...
        with patch(
            ...
            assert len(json_data["orig_local_archives"]) == 0
            assert len(json_data["orig_spec_sources"]) == 0
            assert len(json_data["trans_local_archives"]) == 0
            assert len(json_data["trans_spec_sources"]) == 0
            assert len(json_data["transformations"]) == 0
